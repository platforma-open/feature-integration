...
// Per-sample report, opened from the Main grid on row double-click. Mirrors
// blocks/mixcr-clonotyping's SampleReportPanel: this file is the host that picks what the reader is
// looking at, and each view lives in its own child component.
const sampleId = defineModel<string | undefined>();

// The panel reads the same per-sample view model the grid renders, rather than going back to the raw
// model outputs. That is what keeps the panel's numbers and the grid's Quality / Read recovery columns
// from being two independent derivations of the same QC row.
const sampleData = computed(() => {
  if (sampleId.value === undefined) return undefined;
  return sampleResults.value?.find((result) => result.sampleId === sampleId.value);
});

// Visual Report is the default tab, as in mixcr-clonotyping: opening a finished sample should land on
// what happened to its reads, not on a log the reader has no reason to read once the sample is green.

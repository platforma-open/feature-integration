...
// The Log tab of the per-sample report: the live per-step logs across the whole pipeline (parse /
// refine / tag-stat + the Python per-cell-metrics step). Mirrors blocks/peptide-extraction's
// SampleReportPanel (Logs tab).
//
// This is a plain prop rather than a model because the log view only ever reads the selection —
// nothing here can change which sample the slide-over is showing.

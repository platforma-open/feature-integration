...
//
// `PlChartHistogram` rather than `GraphMaker`: a reader here is asked one question -- do these two humps
// stand apart -- and every control a builder offers is a way to stop answering it. It also takes a
// `threshold`, which is how a declared gate draws its marker; a builder has no marker mechanism at all.
//
// Two forms, picked by `scale`. `log-bins` takes the bins as given and draws a symlog axis with decade
// ticks. `basic` is the only linear axis the component has, and it bins raw values itself, so linear callers
// hand it values reconstructed from the same bins.
//
...
const props = defineProps<{
   ...
   * typed in the same units, which a log axis puts where the reader cannot find it.
   */
  scale?: "log" | "linear";
  title?: string;
...
// read as no gate at all. The smallest positive double sits at the same pixel as 0 on either scale.
...
// carries would draw the reconstruction's interpolation as if it were data.

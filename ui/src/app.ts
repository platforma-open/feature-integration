...
export const sdkPlugin = defineAppV3(platforma, (app) => {
  ...
  // data.defaultBlockLabel, which the sidebar subtitle reads. The subtitle render context has no result pool,
  // so the dataset label is resolved in the output and copied here. Guarded to write only on change, which is
  // multi-client safe.
  watchEffect(() => {
  ...
  // to the CSV or the feature-name column (see MainPage.vue's clearControlOnInputChange). Never here, through
  // a watcher on the controlOptions output: that is the spec-facts-resync hairpin, an output-to-data write
  // (see hairpin.md and model.md). A gesture-driven data-to-data write has no multi-client interleave and no

...
/**
 * The clonotype's readable name (`C-ZDKEZ`) and its cell count, for the one place that cannot get them from a
 * table: the expansion's title.
 *
 * The name is real stored data, not something this block derives. Upstream emits it as a `pl7.app/label`
 * column on the clonotype axis, generated by `mixcr-clonotyping/workflow/src/clonotype-label.lib.tengo`. A
 * TABLE shows it without being asked -- `PlAgDataTableV2` detects label columns and substitutes them into
 * axis cells, which is why the card's `Clone Id` column reads `C-ZDKEZ` while the row event that opens the
 * panel carries only the raw key.
 *
 ...
 *
 * Lifted from `blocks/3D-Structure-Based-Liabilities/ui/src/composables/useClonotypeLabels.ts`, which builds
 * a detail-modal title the same way, and `3d-structure-prediction` does it too. Both take the whole column
 * rather than one key, and so does this: it is one value per clonotype and it leaves the map ready for the
 * next one. `getSingleColumnData` accepts filters if a large run ever makes that the wrong trade.
 */
export function useClonotypeLabels(
  ...
  /**
   * One column's values, keyed by axis value. Returns an empty map rather than throwing: a missing title is a
   * cosmetic loss, and the caller has a generic word to fall back on.
   */
   ...
   * key. That key is the string this exists to keep off the screen: it appears nowhere else in the block, so
   * it names nothing and reads as a bug. A caller with no name says something generic instead.
   *

...
- 990723f: Median antigen depth per clonotype, one column per barcode
...
- Updated dependencies [990723f]
  - @platforma-open/milaboratories.feature-integration.per-cell-metrics@3.7.0

## 3.6.0

### Minor Changes

- 8a94159: Quality Checks: one generation, judged where a line can be defended
  ...
  **Two numbers were wrong, and both are corrected.**
  ...
  _Reads rescued by barcode correction_ is a subtraction, and its subtrahend was
  ...
  _Mean reads per V(D)J-matched cell_ divided **every** matched read in the library by the count of
  ...
  sixteen-fold swing, warning only where V(D)J recovery was _good_. The numerator is now the reads
  ...
  **Four new thresholds, and one moved.** Only the first is traceable to a vendor number:
  ...
  - _Reads matching a panel barcode_ — warn below 0.50, alert at 0. INHERITED: the complement of Cell
  ...
  - _Reads matching the read pattern_ — warn below 0.90, alert below 0.50. A newly declared row; the
  ...
  - _Reads rescued by barcode correction_ — warn above 0.05, alert above 0.10. Both ends face the same
  ...
  - _Median antigen count per V(D)J-matched cell barcode_ — warn below 4, alert **at** 1. The warn is
  ...
  - _Reads whose cell barcode passed quality correction_ moved from 0.75 / 0.50 to 0.95 / 0.75, and
  ...
  **Rows that changed shape.**
  ...
  _Sticky cells, or the spread of control readings_ was one row whose meaning depended on a setting;
  two of its three branches only restated that setting. It is now two rows — _Cells set aside as
  sticky_ and _Median control-tag reading per V(D)J-matched cell_ — both narrowed to the V(D)J-matched
  ...
  _Reads whose cell barcode passed quality correction_ now states its two counts —
  ...
  _Reads parsed_ and _Median antigen count per observed cell barcode_ leave the list. The first is a
  ...
  _Median antigen count per cell_ used to carry eleven decile points in two p-columns that nothing
...
- Updated dependencies [8a94159]
  - @platforma-open/milaboratories.feature-integration.per-cell-metrics@3.6.0
...
- Updated dependencies [bf43ed8]
  - @platforma-open/milaboratories.feature-integration.per-cell-metrics@3.4.0
...
- Updated dependencies [7feac4c]
  - @platforma-open/milaboratories.feature-integration.per-cell-metrics@3.0.1
...
- Updated dependencies [7923317]
  - @platforma-open/milaboratories.feature-integration.per-cell-metrics@3.0.0
...
- Updated dependencies [88d6e9b]
  - @platforma-open/milaboratories.feature-integration.per-cell-metrics@2.1.2

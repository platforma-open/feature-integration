...
membership below is written out by hand, because each one is load-bearing against a threshold. The seed
is passed to `random.Random` rather than seeding the module, so the bed regenerates byte-identically.

Everything here is invented. The repository is public: no real barcode sequence, antigen name or sample
identifier may appear. Barcodes are drawn from ACGT, antigens are `AgNN`, samples are `SNN`.
...
Against a comparator of 60 it is 0.1 at 500 and 100 at 5000.
"""
...
# it plays in the panel, so a regenerated bed with different sequences still exercises the same shapes.
ANTIGEN_SLOTS = ["A0", "A1", "A2", "A3", "A4", "A5", "A6", "A7"]
CONTROL_SLOTS = ["R0", "R1"]

# (sample, antigen name, barcode slot). Panels of 3, 4, 4 and 5 tags across four samples.
#
#   A0, A1, A3, A4 each recur under two different names. That is the case that makes name-keyed identity
...
#   A5 is declared by S03 alone and, see COUNTS, read in S02 alone. That is the only way both directions
#       of the panel-versus-reads check can be seen to run per sample rather than globally.
PANEL = [
    ("S01", "Ag01", "A0"),
    ("S01", "Ag02", "A1"),
    ("S01", "Ag03", "A2"),
...
# and not another is discarded by `consistent_properties`, so a tag is a comparator everywhere or nowhere.
CONTROL_NAMES = {"R0": "Ctrl1", "R1": "Ctrl2"}

# (sample, cell, barcode slot, umiCount).
#
# R0 reads 6 in every cell but c11, where it reads 400. The bed is run with --gate-threshold 100, so c11
# is set aside by the admissibility gate and every identity its set was offered reads *unreliable*. That
# is the bed's only per-cell source of the fourth state.
#
# It used to read 1 instead, which fell below a thin-reference line that routed the cell to *unreliable*.
# That branch was deleted -- no published line separates a thin comparator from a usable one -- so the
# state now comes from the gate.
#
# R1 reads 60, above R0 everywhere it appears, so the two-control panel's comparator is 60 and not 6.
# 60 also sits below the high-reference observation line of 100, keeping that measurement quiet. c11 has
# no R1 row, so it stays impossible to compare on the two-control panel too.
#
# 8 is the *not bound* count: above the floor of 4, so the reading survives to be compared, and 0.0001
# against a comparator of 6, so it is compared and fails. A count of 2 would be zeroed by the floor.
# 500 is *bound* against a comparator of 6 (score 100) and *not bound* against 60 (score 0.1). 5000 stays
# bound against either.
COUNTS = [
    ...
    ("S01", "c03", "A0", 5000),
    ("S01", "c03", "A1", 8),
    # c03 has no A2 row: a cell that was asked and read nothing. It is comparable, so it votes *not bound*
    # against the two cells that bound A2, and the majority still says bound.
    # -- S02, set K02.
    ("S02", "c04", "R0", 6),
    ("S02", "c04", "R1", 60),
    ("S02", "c04", "A0", 500),
    ...
    ("S02", "c04", "A3", 500),
    ("S02", "c04", "A4", 8),
    # A5 is read here and declared only by S03. The reading is real and the panel still says S02 was never
    # asked, which is why the mismatch table has to travel with the answer.
    ("S02", "c04", "A5", 500),
    ("S02", "c05", "R0", 6),
    ("S02", "c05", "R1", 60),
    ("S02", "c05", "A0", 500),
    ...
    ("S02", "c06", "A0", 500),
    ("S02", "c06", "A1", 8),
    # 2 is below the floor of 4 and is zeroed, so this is the bed's floored reading and c06 votes *not
    # bound* on A3 while c04 and c05 bind it.
    ("S02", "c06", "A3", 2),
    ("S02", "c06", "A4", 8),
    ...
    ("S03", "c07", "A2", 500),
    ("S03", "c07", "A3", 8),
    # No A5 row in S03 at all, though S03 is the only sample that declares it: the other direction of the
    # same check. Both S03 cells were offered A5 and read nothing, so K03 reads *not bound* at A5.
    #
    # c08's comparator reads 1: below the floor of 4, but the floor spares a DECLARED comparator, so it
    # survives to be compared -- 0.96 against 8, and 100 against 500. Read without a declaration it is
    # floored like any other count, which is what makes the exemption observable.
    ("S03", "c08", "R0", 1),
    ("S03", "c08", "R1", 60),
    ("S03", "c08", "A0", 500),
    ("S03", "c08", "A2", 500),
    ...
    ("S04", "c09", "A1", 8),
    ("S04", "c09", "A4", 500),
    # A6 and A7 are the two barcodes of Ag07, and the two S04 cells of K03 bind opposite ones. Read per
    # barcode each splits its set one to one and reads *unreliable* on the tie. Read as one antigen by the
    # highest member, both cells bind Ag07 and the set reads *bound*.
    ("S04", "c09", "A6", 500),
    ("S04", "c09", "A7", 8),
    ("S04", "c10", "R0", 6),
    ("S04", "c10", "R1", 60),
...
# (sample, cell, set). K03 spans two samples on purpose; K04 is a singleton, which many real clonotypes
# are.
LINKER = [
    ("S01", "c01", "K01"),
    ("S01", "c02", "K01"),
...
# and `--reference-values` has nothing correct to name. The `Type` column of the wide shape declares what
# a member is TO THE QUESTION, which is a different axis from what a count is read against. Naming
# `Off-Target` as the comparator holds every off-target out of the identity universe, so the question an
# off-target exists to pose is deleted.
...
# Case variants, because the observed file carried six values that were three roles. Two failure modes,
# kept separate so a test can tell them apart:
#
#   A0 reads "Target (Primary)" in S01/S03 and "Target (primary)" in S02/S04. One barcode, two values, so
#   the property is dropped for that tag entirely.
#
#   A5 reads "Off-target" everywhere it appears. Self-consistent, so it keeps its role, but it no longer
#   matches A2's "Off-Target" -- so selecting one role value silently misses the other.
...
def _typed(slot: str, sample: str) -> str:
    ...
    if slot in ALWAYS_LOWERCASED or sample in LOWERCASED_IN.get(slot, set()):
        # Lowercase only the parenthesised qualifier or the word after the hyphen, which is how the observed
        # variants differed -- not a blanket .lower().

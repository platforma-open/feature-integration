...
def test_a_cell_holding_only_the_reference_is_not_emptied():
    # Its non-reference readings are absent, not zeroed. "Emptied" means the floor took a cell's
    # evidence away, not that it never had any.
...
def test_the_same_cell_id_in_two_samples_empties_independently():
    # (sampleId, cellId) is the key. Keying on cellId alone would let S2's surviving reading rescue
    # S1's emptied cell.
...
def test_a_disabled_floor_is_a_no_op_even_for_a_zero_reading():
    ...
    # Falling through would count a cell whose only reading is already 0 as "emptied".
...
def test_a_reading_that_was_already_zero_still_counts_as_evidence_lost():
    # Pins the deliberate asymmetry between had_evidence and kept_evidence. had_evidence must NOT filter
    # on > 0: on the sparse frame this step receives, every row is an observed reading, so a row's
    # existence is what makes a cell one that had evidence. Adding "> 0" is a no-op on real input and
    # silently changes this count once densified zeros exist.
    df = _counts([("S1", "c1", "AAAA", 0)])
    _, stats = apply_floor(df, floor=4, reference_tags=set())
    assert stats["cellsEmptied"] == 1
...
def test_nothing_here_picks_a_rung():
    # The scientist selects among the rungs, and nothing selects for them.
    #
    # A tripwire rather than a permanent ban: wanting a default again should be a deliberate act that
    # deletes this test. The workflow omits --reference-source where the model's value is empty, so
    # anything here that could pick a rung becomes the live rule the moment the model stops picking one.
    import verdict
...
def test_a_choice_that_cannot_serve_refuses_and_never_falls_to_another_rung():
    # `served_source` survived the removal above and is a different thing: it never picks a rung. There
    # is no rung below to fall to either, so it refuses and names the condition that failed. Both
    # conditions are properties of the settings, knowable before anything is read.
...
def test_there_is_no_bottom_rung():
    # A tripwire. Rung five was "no baseline exists, every verdict unreliable", and it is gone: a full
    # punchcard of non-answers costs what a real run costs and looks like a result at a glance.
    assert not hasattr(ReferenceChoice, "NONE")


def test_empty_droplets_is_not_offered():
    # A tripwire, not a permanent ban: the day this block genuinely receives gene expression and an
    # empty-droplet population, EMPTY_DROPLETS gets implemented and this test is deleted.
...
def test_one_reference_tag_still_serves():
    # The supported shape, and the one every antibody kit ships: one comparator for the whole panel.
...
def test_several_reference_tags_do_not_block_a_rung_that_does_not_use_them():
    # The refusal is scoped to the rung that reads a declared tag AS the comparator. Under the panel rung
    # several declared tags are just readings in the median, which is well defined however many there are.
...
def test_a_declared_rung_with_no_declared_tag_refuses():
    # The refusal reaches the caller through `reference_by_cell` too, since that is where a hand-driven
    # run enters. The message names the condition and what to change.
...
def test_shipped_defaults_are_pinned():
    # These are user-facing numbers that appear in a dropdown and change what the block produces, so an
    # edit to any of them must be a deliberate, visible act. The high-reference line is not calibrated
    # against real data.
    #
    # The panel minimum is different in kind: it GATES the rung rather than tuning it, and it comes from
    # one preprint whose own panels held fifty and a hundred members. At 25 the rung is out of reach of
    # any antibody panel, since those kits cap at fifteen tags.
    assert DEFAULT_PANEL_MIN_MEMBERS == 25


def test_panel_source_serves_exactly_at_the_minimum():
    # The minimum is a floor, not a gap: a panel of exactly min_members is large enough. Nothing else in
    # the suite distinguishes < from <=.
...
def test_panel_source_refuses_one_below_the_minimum():
    # A gate rather than a preference: below it the baseline the rung permits is not conservative but
    # wrong, so the run refuses rather than serving it.
    counts = _counts([("S1", "c1", "AAAA", 9)])
...
def test_no_gate_counts_nothing_rather_than_counting_zero():
    # None, never 0. A zero would report a run in which no cell read high, which is a finding. With no
    # gate there is no *high* to read against at all, and the caller reports the spread instead.
    ref = {("S1", "a"): 500, ("S1", "b"): 50, ("S1", "c"): 2000}
...
def test_a_source_that_cannot_be_served_refuses_and_never_moves_sideways():
    # A rung that cannot serve refuses. It never substitutes another, which would silently answer a
    # question the scientist did not ask.
    counts = _counts([("S1", "c1", "AAAA", 9), ("S1", "c1", "CCCC", 3)])
    # DECLARED with nothing declared: refused, not served by PANEL, even though a panel of a hundred
    # tags sits right there and would satisfy that rung.
    with pytest.raises(SystemExit, match="declares no baseline tag"):
        reference_by_cell(counts, set(), ReferenceChoice.DECLARED, panel_size=100, min_members=5)
    # PANEL below the minimum: refused, not served by DECLARED, even though a reference tag exists.
    with pytest.raises(SystemExit, match="below the 5 that rung needs"):
...
def test_the_panel_comparator_is_the_median_not_the_mean():
    ...
    # stands for the cell's background, so a single high reading must not raise the bar.
    counts = _counts(
        [
            ("S1", "c1", "AAAA", 1),
            ("S1", "c1", "CCCC", 2),
...
def test_the_panel_median_truncates_rather_than_rounds():
    ...
    # apart. Truncation is the behaviour, and a comparator of 1 and one of 2 give different scores.
    counts = _counts(
        [
            ("S1", "c1", "AAAA", 1),
            ("S1", "c1", "CCCC", 1),
...
def test_cells_outside_the_given_list_are_excluded():
    ...
    # comparator nobody will consult.
...
def test_a_very_low_comparator_is_scored_rather_than_rerouted():
    # The thin-reference branch was deleted rather than filled in. No published line separates thin from
    # usable, so the comparison runs and the reference reading is emitted for the reader to judge. A
...
def test_a_gated_cell_reports_the_gate_even_when_its_reference_is_very_low():
    # The gate set this cell aside AND its comparator reads 1. A very low comparator is no longer a reason
    # on its own, so only the gate can be reported. The reason is an exported column a later step reads to
    # tell a panel problem from a re-run problem.
...
def test_a_score_exactly_at_the_cutoff_is_bound():
    # The named value satisfies the condition it names. Integer counts have no rational preimage of a
    # fixed cutoff like 75.0 under the beta CDF, so the exact boundary is built the other way round:
    # compute a reading's own score, then feed that exact value back in as the cutoff.
...
def test_no_line_separates_a_thin_comparator_from_a_usable_one():
    # There is no boundary left to be off-by-one about. A comparator of 1 and one of 2 differ only in the
    # score they produce, and a large enough antigen count binds against either.
    for ref in (1, 2):
...
def test_no_comparator_is_unreliable_but_a_comparator_reading_zero_is_scored():
    # The two must not collapse. served=NONE, modelled here as an empty reference dict, means no
    # comparison existed. A comparator present and reading 0 is a real comparison and scores normally.
    # This also subsumes the plain no-comparator-is-unreliable check.
...
def test_silent_admissible_cell_can_never_score_bound():
    # The fact the analytic path rests on: specificity_score(0, r) is ~0.0422 at r = 0 and smaller for
    # every larger r. A silent admissible cell is therefore always *not bound* for any cutoff above that
    # bound, which is what lets silent_tally skip materializing its row.
...
def test_duplicated_cells_rows_give_the_deduped_answer():
    # A row-count bug this project shipped once already: keys built from `cells` without dedup counted
    # the duplicated c2 row as if it were a second cell. asked must count distinct cells (2), not rows.
...
def test_duplicated_observed_rows_are_rejected_not_silently_wrong():
    ...
    # returned silentUnreliable == -1. A duplicated observed row for an inadmissible cell is counted twice
    # against a total that counts the cell once. `observed` must be unique on (cell, identity).
    cells = _cells([("S1", "c1")])
    ...
    # ValueError rather than AssertionError, and the type is the point: an `assert` is stripped under -O,
    # and this guard stripped does not crash -- it returns a wrong answer.
    with pytest.raises(ValueError):
        silent_tally(observed, cells, {"S1": {"A"}}, admissibility)


def _build_silent_tally_population(seed, force_empty_sample=None):
    # Shared by every check below: build a small, varied population by construction -- several samples,
    # cells, identities, some cells gated, some with a very low reference, some with a normal one.
    ...
    for sample in samples:
        ...
        for i in range(6):
            ...
            key = (sample, cell)
            # Reference reading: sometimes missing (no comparator), sometimes very low, sometimes ordinary.
            roll = rng.random()
            if roll < 0.2:
            ...
            if rng.random() < 0.15:
                gated.add(key)
            # Sparse observed readings: only some (cell, identity) pairs the sample offered got a row.
            for identity in offered_by_sample[sample]:
                if rng.random() < 0.5:
                    # Some readings must actually clear the cutoff. Against references of 2-20 the largest score a
                    # count of 30 can reach is about 11.8, so a population drawn only from 0-30 contains no bound
                    # cell at all and the oracle comparison below would read 0 == 0 in every run.
...
def _check_silent_tally_matches_oracle(seed, cutoff=BOUND_CUTOFF, force_empty_sample=None):
    # Checks silent_tally's three cheap terms, sample-keyed (the default), against the dense grid built
    # by densify and read through read_states, which never skips a row.
    ...
    # A tally that emits extra rows -- one for an identity a sample never offered -- must fail here, not
    # just disagree on counts.
    ...
    for sample in samples:
        ...
        for identity in identities:
            ...
            # A silent admissible cell can never be observed as bound, so the oracle and the sparse frame
            # must agree exactly on bound counts.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (5, None),
        (6, None),
        # The generator above never draws an empty offered set on its own. Force one so the empty-block
        # path in densify and the zero-row case in silent_tally are both exercised against the oracle.
        (7, "S2"),
    ],
)
...
def test_silent_tally_agrees_with_the_oracle_at_a_low_valid_cutoff():
    # 0.5 is comfortably above specificity_score(0, 0) ~= 0.0422. This guards the boundary itself rather
    # than assuming BOUND_CUTOFF=75.0 is representative of every cutoff the equivalence must hold for.
...
def _check_silent_tally_matches_oracle_grouped(seed, cutoff=BOUND_CUTOFF, force_empty_sample=None):
    # Same population and same dense oracle as the sample-keyed check above, but the cells are regrouped
    # into sets that mix samples with different offered identities. A set's cell index (0..5) becomes its
    # group, independent of sample, so every group contains a member from all three samples. That is
    # exactly the shape a hoisted asked/total_inadmissible would get wrong.
    samples, identities, gated, reference, cell_rows, tag_rows, offered_by_sample = _build_silent_tally_population(
        seed, force_empty_sample
    )
    ...
    for group in groups:
        ...
        for identity in identities:
            ...
            if identity not in offered_here:
                # None of this group's members' own samples offered it: no row at all, not a zero row.
                assert tally.filter(group_filter & (pl.col("identity") == identity)).height == 0
                continue

            def _states_for(frame):
                # A plain Python filter, not a polars struct comparison. This only needs to run over a handful
                # of rows, and it sidesteps any doubt about how polars compares struct columns.
                return [
                    state
...
def test_a_group_spanning_two_panels_does_not_inflate_silent_unreliable():
    # THE hoist bug, pinned directly: S1 offers A, S2 does not. A group holds one cell from each, and
    # S2's cell is gated. A hoisted total_inadmissible would count S2's gated cell against identity A too,
    # inflating silentUnreliable for an identity that cell was never asked about.
...
def test_same_barcode_combines_into_different_identities_by_sample():
    # The cell's own sample decides which antigen its barcode counted toward. Keyed by tag alone this put
    # both cells under whichever identity the dataset-wide map happened to hold.
...
def test_combine_is_still_the_max_within_one_sample():
    # The combination rule is unchanged by the keying: two tags of one identity in one cell still combine
    # to the highest, never the sum.
...
def test_the_comparator_is_computed_on_raw_counts_not_floored_ones(tmp_path):
    ...
    The bug this pins was at the call site, not in this module: production handed `reference_by_cell` the
    FLOORED frame. A cell whose panel readings straddle the minimum medians differently before and after:
    [1, 1, 2, 9, 9] medians to 2, and the same readings floored at 4 are [0, 0, 0, 9, 9], which medians to
    0. A comparator of 0 rather than 2 moves every one of that cell's verdicts toward *bound*.

    The median was also internally inconsistent wherever a reference tag was present, because the minimum
    exempts reference tags and floors every antigen tag, so one median ran over a mixture of raw and
    floored values.
    """
    raw = _counts(
        [
    ...
    # Held so the test fails loudly if the minimum ever stops biting here -- the two frames must actually
    # differ, or this proves nothing.
    assert on_floored[("S1", "c1")] == 0


def test_the_minimum_spares_the_baseline_tag_by_default():
    # The shipped behaviour. The minimum removes what is not evidence OF BINDING, and the comparator is
...
def test_the_minimum_never_reaches_the_baseline_tag():
    # A rule and not a preference: the minimum asks whether a count is evidence of binding, and a tag
    # declared to be bound by nothing never is. There is no switch. A below-minimum comparator survives,
    # and the count of removed readings counts the antigen tag alone.
    counts = _counts([("S1", "c1", "CTRL", 2), ("S1", "c1", "AAAA", 2)])
    out, stats = apply_floor(counts, floor=4, reference_tags={"CTRL"})
    by_tag = dict(zip(out["tag"].to_list(), out["umiCount"].to_list(), strict=True))
    assert by_tag == {"CTRL": 2, "AAAA": 0}
...
def test_a_cell_holding_only_an_exempt_comparator_is_not_emptied():
    # A cell holding ONLY a below-minimum comparator. The comparator is exempt, so the cell never had
    # evidence of binding removed and the minimum emptied nothing. Scoping the emptied population one way
    # while flooring the other would report a cell as losing evidence it never held.
...
def test_the_minimum_changes_no_comparator():
    # Pinned at the source. Every rung reads its own counts raw, so the comparator is built from the
    # unfloored frame and the minimum cannot reach it.
...
def test_a_cell_whose_comparator_survived_read_something():
    # The whole discriminator. A cell whose antigen tags all fell below the minimum while its comparator
    # survived took up reagent and none of it was antigen: a real negative and a real vote, not an empty
    # cell. `cellsEmptied` cannot see this, because with the comparator exempt it is scoped to the readings
    # the minimum was allowed to remove.
...
def test_a_cell_keeping_only_its_comparator_did_not_read_nothing():
    # The comparator is always exempt, so this cell keeps a reading. It did not read nothing, even though
    # nothing it holds is evidence of binding.
...
def test_the_universe_bounds_the_answer():
    # A cell outside the universe does not belong here however it read. This is what keeps the count from
    # ever exceeding the clonotype's own cell count.
    counts = _counts([("S1", "c1", "AAAA", 500)])
    floored = apply_floor(counts, 4, set()).counts

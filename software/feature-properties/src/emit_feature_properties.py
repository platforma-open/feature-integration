...
The user maps two columns to roles: the feature-barcode sequence (the join key) and the feature name
(the antigen identity). Every OTHER column is an arbitrary per-feature property -- antigen type,
species of origin, pool. This step imports each generically, with no hardcoded schema, so the
workflow can surface it as a p-column keyed on the feature axis.

Two outputs, stdlib only:
  * ``<prefix>_feature_properties.csv`` -- one row per distinct feature. Columns are ``feature``
    (always renamed, so the downstream xsv import keys it on the feature axis) followed by every
    extra column under its own header. Sorted by feature.
  * ``<prefix>_feature_property_meta.json`` -- ``{columns, valuesByColumn}``: property names in header
    order, and each property's distinct non-empty values, sorted.

Deduplication: a barcode can map many-to-one onto a feature, so a feature appears on several rows.
Properties are intrinsic to the feature and are expected to agree. The FIRST non-empty value in file
order wins, and a differing later value is reported to stderr but is not fatal -- values are imported
as given and are not validated. Output is fully deterministic for a given input file.
"""
...
def _read_header(reader: "csv._reader") -> tuple[list[str], dict[str, int]]:
    """Return (ordered named columns, name -> first column index). Blank header cells are dropped, and
    the first index wins for a duplicated header. Read by position so order is preserved --
    DictReader would collapse duplicates and lose order."""
...
def parse_properties(
    ...
    ``by_feature[feature][property]`` is the first non-empty value seen for that pair, in file order.
    ``values[property]`` is the set of distinct non-empty values. A later row that disagrees with a
    recorded non-empty value is reported to stderr and kept, not overwritten.
    """
...
def main() -> None:
    ...
    # Role columns are excluded, and every remaining named column is a property. The sample column is a
    # role too, so it is excluded when set.
    ...
    # Wide per-feature CSV: feature plus one column per property, one row per feature, sorted, missing
    # values blank. The feature-name column is always emitted as 'feature', so the xsv import keys it on
    # pl7.app/feature/featureId whatever the source header.
    ...
    # Meta: property names in header order plus each property's distinct sorted values. The workflow
    # builds one import column per name and puts the values into its discreteValues.
    ...
    # Multiomic Integration removes those controls ENTIRELY from its antigen metrics. An off-target, by
    # contrast, stays in the metrics. Header-only when no control is designated.
    #
    # SEVERAL controls are allowed: being a control is a property of the tag, where being the reference
    # that supplies the baseline is a job given to exactly one of them. This file marks controls and never
    # nominates a reference. Duplicates are dropped and the given order kept.
    seen: set[str] = set()
    controls = []

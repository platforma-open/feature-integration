 ...
 *  blocks/peptide-extraction uses. A fixed-kit preset carries its `pattern` directly. A user-configurable
 *  preset leaves `pattern` empty and drives it from `data.pattern` in the UI builder. */
export type Preset = {
  id: string;
  vendor: string;
  kit: string;

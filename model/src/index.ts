...
// background estimate. Keep it above the fifteen-tag cap of an antibody kit, so such a panel falls to
...
// id is mangled by `substituteSpecialCharacters`, and a substring test lets `SpikeWT` match
...
// The clonotype's cell count, carried in the punchcard's own frame so the grid can read it. A block's own
// exports are not in its own result pool, so the copy in the exported setCounts family is unreachable
// from here.
export const PUNCH_CELL_COUNT_COLUMN = "pl7.app/antigen/cellCount";

// How each comparator choice is written for a reader, and the single place that wording lives. The Python
// enum, the run-meta JSON and the p-column domain all carry the machine token, so rewording a sentence
// here cannot break a branch. These strings match the labels `referenceSources` offers before a run, so a
// choice does not change its name once it has served. User-facing names only: the DATA layer keeps
// `declared`/`panel`/`none`, which are p-column domain values, and domain is part of column identity.
...
export type VerdictRunMeta = {
   ...
   * reports afterwards. The other rungs are refused from the settings before anything is read.
   *
   * False means the run finished and read no verdicts. The punchcard is not drawn, and the reason is
   * shown in its place: a full grid of *unreliable* costs what a real run costs and looks like a result
   * at a glance.
   */
  ...
  /**
   * How many DISTINCT panels the run carried. One means every sample was stained with the same tags, and
   * then how many of a clonotype's cells could answer does not vary by identity -- it is the clonotype's
   * own cell count, which the grid carries beside its name. Optional because a run record written before
   * this field existed does not have it, and a reader treats absent as one.
   */
  samplePanelCount?: number;
  /**
   * The read limit the run applied, or absent/null where none was declared. Present because it is the one
   * signal for "a gate was declared" -- `the-explore-readout` shows set-aside cells only then, and a gate
   * that set nothing aside must still say so rather than look like no gate at all.
   */
   ...
   *
   * Set grain, deliberately, and not a column of the expansion table: a set-aside cell answers nothing at
   * any identity, so repeating the subtraction at every position would imply a per-identity failure that
   * did not happen.
   */
...
// mitool prefixes its progress lines with this marker, set via MI_PROGRESS_PREFIX in the workflow step
// templates. The model scrapes matching lines for a live per-sample 0-100% bar, and ProgressPattern pulls
// the stage name, percent and ETA out of one. Same values as blocks/peptide-extraction.
export const ProgressPrefix = "[==PROGRESS==]";
export const ProgressPattern =
  /(?<stage>[^:]*):(?: *(?<progress>[0-9.]+)%)?(?: *ETA: *(?<eta>.+))?/;

// Per-sample QC metrics as emitted by qc_report.py (result_qc.json), read by the analysisLog output and,
// per sample, by the Main grid's Quality + Read recovery columns (derived in ui/src/results.ts).
export type QcRow = {
  readsTotal: number;
  readsMatched: number;
...
// count/totalWeight/unique_<UMI> outputs. A user-mapped CSV barcode/feature column naming one of these
// would corrupt the join or crash group_by in per_cell_metrics.py -- which guards it too, but only after
// the full mitool chain has run. args() rejects it here so Run is disabled up front.
...
// sample axis. Shared by the sampleLabels and analysisLog outputs. A module helper rather than one output
// reading another, because each block output is an independent pure function of ctx.
function resolveSampleLabels(
  ...
  if (axisKeys0 !== undefined) {
    try {
      ...
      datasetSampleIds = undefined; // malformed: do not scope, fall back to the full map below
    }
  }
  ...
  // Restrict to the selected dataset's samples, falling back to the full map if the annotation was missing.
...
// finished. Shared by completedSamples / sampleQc / analysisLog. Returns [] when outputs have not settled.
// A caller that must tell "not started" apart maps that to undefined itself.
...
// `referenceRungsAvailable` stood here and is gone with the derivation it fed. Which rungs this data could
// serve is still worth SAYING -- the dropdown's option list says it -- but it must not decide anything. A
// helper shared between a display and a projection is how the deciding crept back in.
 ...
 * experiment would otherwise be answered by different rules. There is exactly one place a rung comes
 * from: `data.referenceSource`. Never derive it from what the panel can serve.
 *
 * An unselected run IS refused, and undefined is what carries that. There is no bottom rung answering
 * every position *unreliable* -- that output is honest and useless, costing what a real run costs while
 * looking like a result at a glance. `args()` throws on undefined. This function only reports it.
 *
 ...
 * software otherwise. Nothing here writes to `data`, so a choice that becomes serviceable again revives
 * on its own.
 */
...
// does, or when the CSV meta has not resolved. Blank cells are ignored rather than counted against the
// column: a trailing empty row is a CSV artefact, not evidence about the contents. A module helper because
// a block output cannot read another output, and two need this -- barcodeAlphabetIssue reports it, and
// barcodeMappingIssue stays silent while it holds, so the two never hand the reader contradictory fixes.
function barcodeAlphabetProblem(
  ...
  // Name a column that would work, if the CSV has one. The mistake is nearly always "picked the ID column
  // when the sequences are one over", so naming the alternative saves the user a guess.
...
// when the same barcode maps to different features per sample, and the tell is a column whose distinct
// values cover the dataset's sample names. Returns the column whose distinct values are a SUPERSET of the
// dataset sample names, preferring exact set-equality then fewest extra values, excluding the columns
// already bound to the barcode / feature roles. Shared by the suggestedSampleColumn output and
// barcodeMappingIssue. Undefined until both the CSV meta and sample labels resolve.
...
// v2 data shape: the preset selector and pattern string, with the dominance-era parameters still on it.
...
// preset selector and a mitool tag-pattern string (see model/src/pattern.ts, model/src/presets).
...
const dataModel = new DataModelBuilder()
  ...
  .migrate<BlockDataV2>("v2", ({ cellLen, umiLen, featureLen, ...rest }) => {
    ...
    // preset carrying the assembled pattern -- offset 0, the only layout the v1 UI could express.
  ...
  // filter is meaningful only against the frame it was saved on. The punchcard's own state starts fresh,
  // on the whole panel.
  //
...
export const platforma = BlockModelV3.create(dataModel)
  .args((data): BlockArgs => {
    ...
    // Reject a CSV column colliding with a reserved tag-stat column up front, for the same reason as the
    ...
    // Optional combine-mode column: its values are per-feature modes ("sum"/"all"), so it must be its OWN
    // CSV column -- distinct from the barcode-sequence and feature-name roles, and not a reserved tag-stat
    // column. Python guards this too, but only after the mitool chain runs. Reject up front so a mis-picked
    // column, such as the barcode column of DNA sequences, disables Run with a clear message.
    ...
    // Sample-aware mapping, optional: when a sample column is chosen, the per-sample workflow body filters
    // the CSV to its own sample's rows. Pass the column name and the sampleId->name snapshot it needs to
    // translate its iteration key. That snapshot is taken on the same gesture that sets the column
    // (MainPage.setSampleColumn). Require it here so a half-set state disables Run.
    ...
    if (sampleAware) {
      ...
      // Block Run when a dataset sample has no rows in the CSV's sample column -- it would silently get no
      // features. Gate purely from the snapshots taken when the column was picked, since args is data-only.
    ...
    // The reading's own parameters. The single-cell V(D)J dataset is deliberately NOT required: without it
    // the block still emits the tag counts, the per-cell scalars, the panel-versus-reads check and the
    ...
    // reference_values`, so it is validated, recorded in the run meta, and changes no number. The run then
    // reads against the panel's own readings while the form says a baseline tag is declared -- a wrong
    // answer wearing the look of a configured one. Requiring the values costs no expressiveness: a panel
    // that declares no baseline leaves this column blank.
    //
    // A baseline is required, and a run without one does not happen. The alternative is a full punchcard
    // of *unreliable*, which costs what a real run costs and looks like a result at a glance.
    //
    // Refused HERE, before anything is read, because which rung was chosen and whether a baseline tag is
    // declared are properties of the settings. The scientist changes the configuration instead of waiting
    ...
    // which lives in the CSV metadata, and this projection must not read that (see the note on the staging
    // projection below). The software refuses it instead, naming the same condition, and the
    // `referenceSources` output marks the option unserviceable.
    //
    // The cell-count condition is not checked anywhere before the run, and cannot be: whether a sample
    ...
    // on its own: a grouping may name several, and joining them would compare "Identity, Channel" against
    // the panel's headers, match nothing, throw here, and take the whole block to Limbo, refs and all.
    ...
    // First: a column the panel reader consumes as a KEY is not a property column, so naming one here ends
    // the run at the exec. emit_verdicts.py raises on a grouping column the panel does not declare, and on
    // a role column wherever role values are set. Where they are NOT set it raises nothing and the baseline
    // falls back to the panel's own readings -- a wrong answer rather than no answer.
    //
    ...
    // against data rather than the header snapshot, because a key column IS a real header.
    ...
    // Second: a role column or a grouping column the panel does not carry at all ends the whole run at the
    // exec too, and the user meets that as a dead run with no hint of which setting caused it. The check is
    // against the headers snapshotted when the column was picked -- args reads data only -- so a panel swap
    // that leaves the pick behind disables Run with a message naming the column instead.
    ...
    // stale and re-runs the whole reading for nothing. A group of fewer than two members is dropped -- one
    // identity contends with nothing, and an empty group is that same case.
    ...
    return {
      ...
      // the AND per-barcode "fired" floor, an integer >= 1 defaulting to 1, projected only alongside
      ...
      // different one, so what this sends is what the run is answered under. The run record carries both.
      ...
      // record naming `columns` is what every future reader should see.
      grouping:
      ...
      // Optional mitool resource overrides (Advanced Settings). Project only positive integers so a blank or
      // zero field falls through to the workflow defaults (4 CPUs, formula-sized RAM) instead of sending a
      // meaningless request or staling the block on an empty edit.
  ...
  // featureNameColumn is deliberately NOT a prerun arg, and neither is fbFastqRef: the panel is independent
  // of the FASTQ, so keying staging on it would re-import the CSV on every FASTQ change or PlRef re-resolve.
  //
  ...
  // that write cannot re-run staging. Add the snapshot here and it can: the write re-renders staging, which
  // re-exports the blob, which re-triggers the write. And because a staging re-render calls resetStaging
  // first, every turn of that loop would discard the uploaded CSV, not merely waste work.
  .prerunArgs((data) => ({
    tagFeatureCsvHandle: data.tagFeatureCsvHandle,
  }))
  // Enrichments (.enriches): intentionally NOT declared. `.enriches(args => PlRef[])` is for a block that
  // produces columns sharing the key space of a ref it holds (clonotype-browser enriches its inputAnchor,
  // cell-browser its countsRef). This block introduces a NEW cell/feature key space
  ...
  // The single-cell V(D)J dataset the verdicts are keyed by: columns on [sampleId, scClonotypeKey] flagged
  // as anchors -- the same query VDJ Multiomic Integration uses, so the two blocks offer the user the same
  // list. There is deliberately no linkerOptions beside it: the cell linker carries pl7.app/isLinkerColumn
  // and is hidden in tables, so it is not a column a user can pick, and the workflow resolves it from this
  // anchor by name.
  ...
  // are their own labels, because the panel metadata is column-wise and carries no tag->name pairing.
  // Retentive so the editor does not blank on a rerun.
  //
  // This output exists so that only the USER'S PICKS are ever written to data. A watcher copying this list
  // into data would make the output depend on data derived from it, and two open clients would race.
  .retentiveOutput("identityOptions", (ctx): { value: string; label: string }[] => {
    ...
    // values, and the prerun CSV meta is column-wise, with no pairing between columns. Crossing them would
    // invent combinations the panel never declared, and a fabricated identity is worse than none.
  ...
  // current inputs. Computed here rather than in .subtitle because the subtitle context has no result pool.
  // A UI watchEffect copies this into data.defaultBlockLabel. Each part is dropped until set.
  ...
  // that column's values into data, which args gates Run on.
  .retentiveOutput(
    "csvValuesByColumn",
    (ctx): Record<string, string[]> => readCsvMeta(ctx)?.valuesByColumn ?? {},
  )
  // Sample-aware mapping sanity check, a UI warning only -- args is the authoritative gate. When a sample
  ...
  .retentiveOutput("sampleMappingWarning", (ctx): string[] | undefined => {
    ...
    // Extra CSV values are informational, since those rows are never used. Counted into a real plural
    // rather than written "sample(s)": these lines are read while something is already wrong.
    const lines: string[] = [];
    if (missing.length > 0)
      lines.push(
  ...
  // The tag CSV column that looks like it names the dataset's samples, or undefined. The UI offers it as a
  // one-click "use sample-aware mapping" suggestion. Purely advisory: the user must still pick it, the
  ...
  // select, and that choice writes a panel.txt of non-nucleotide strings. The run then fails several stages
  // later, inside barcode correction, with "Error while loading sequence set from ./panel.txt" and a Java
  // stack trace, after the reads are parsed.
  //
  // The args guard cannot catch this: it sees only `data`, and the values live in the prerun CSV meta.
  // Deliberately not gated on sampleColumn -- a per-sample filter narrows which rows reach the panel, and
  // never turns an identifier into a sequence.
  ...
  // set, and that barcode column has fewer distinct values than the CSV has data rows. Some barcode then
  // maps on more than one row, which would fan the per-cell join and double molecule counts. Names the fix:
  // set the Sample column, suggesting the likely one, or remove the duplicate rows. Skipped where rowCount
  // is absent, which defers to the Python guard.
  .retentiveOutput("barcodeMappingIssue", (ctx): string | undefined => {
    ...
    // reader to the sample column. The mistake is in the barcode column itself.
  ...
  // supplies. `sampleMappingWarning` validates a column that has been chosen and returns nothing when none
  // has.
  //
  ...
  // that one is the louder problem and already names this fix.
  //
  // Worth a warning rather than a tooltip because, read as one panel, every sample is offered every
  // antigen. An antigen a sample was never stained with then comes back NOT BOUND instead of NEVER ASKED --
  // the collapse of a non-answer into a negative that the four-state verdict exists to prevent, and nothing
  // else on the page would say it happened.
  ...
  // Those two numbers are what args() needs to refuse a duplicate mapping.
  .retentiveOutput("csvRowCount", (ctx): number | undefined => readCsvMeta(ctx)?.rowCount)
  // True while the panel has been picked but not yet read: the handle is set and no snapshot matches it. A
  // local pick closes this window within a tick. A remote pick holds it until the upload lands and the UI
  // parses the exported blob. Lets the UI show a "reading columns..." state instead of silent empty
  ...
  // middle-layer upload driver, so the CSV bytes are pushed. isActive keeps it computing while the block is
  // not being viewed. Without this the CSV never uploads and every per-sample body hangs on __extra_tagsCsv
  // (mirrors immune-assay-data index.ts / samples-and-data).
  .output(
    "tagFeatureCsvImportHandle",
    (ctx) => ctx.outputs?.resolve("tagFeatureCsvImportHandle")?.getImportProgress(),
    { isActive: true },
  )
  // Same upload driver, resolved from the PRERUN (staging) render, which is the one that fires before Run.
  // The prerun reads the uploaded CSV to populate the CSV-derived dropdowns (csvColumnOptions /
  // controlOptions), and args() REQUIRES their values. The main driver above fires only once args() passes,
  // so on its own it deadlocks: no upload, empty dropdowns, args() throws, no main render, no upload.
  // Driving the upload from staging breaks the cycle (mirrors samples-and-data).
  .output(
  ...
  // This is what lets the UI read the CSV's bytes for a REMOTE (index://) pick, where it cannot read the
  // user's disk: the same client-side parser then runs on these bytes instead. The prerun already exported
  // csvFile to make staging demand the blob, so this adds no work to the workflow. `traverse` rather than
  // `resolve`, because it does not assert a field type.
  ...
  // True once the main workflow has begun producing outputs (ctx.outputs settles), so the Main page can
  ...
  // stream. Drives the rich per-step text in the grid Progress cell: which tag is being corrected, sort vs
  // write phase, live %. ui/src/progress.ts composes these into a MONOTONIC cumulative bar, each step owning
  // a quarter, so it never resets to zero between steps. Same source as stepLogs.
  ...
  // content, so getDataAsJsonOrUndefined reads it synchronously. The done-set drives the grid's "Done"
  // state: a sample not in this set is still Processing.
  .output("completedSamples", (ctx): string[] | undefined => {
    if (ctx.outputs === undefined) return undefined;
    return parseQcRows(ctx).map((e) => String(e.key[0]));
  })
  // Per-sample QC metrics from qcJson, keyed by sampleId, driving the Main grid's Quality + Read recovery
  // columns (derived in ui/src/results.ts). Present per sample once its qc step settles, the same source as
  // completedSamples, so the two columns fill in as each sample finishes.
  ...
  // resolveSampleLabels with analysisLog. Its own output because outputs cannot read one another.
  .output("sampleLabels", (ctx): Record<string, string> | undefined => resolveSampleLabels(ctx))
  // The block's single "Analysis logs", shown in the UI's wide slide-over, built from the per-sample QC
  ...
  .output("analysisLog", (ctx): string[] | undefined => {
    ...
    // While the run is in progress: a live count of samples finished so far. No fixed denominator, because
    // the block processes only the samples present in its feature-barcode dataset, which is not reliably
  ...
  // per-cell summary columns: Max feature UMI count, Max feature fraction, and a "Feature breakdown" string
  // listing every feature as "feature (fraction%, umi)" sorted by descending fraction. The per-feature
  // matrix is still exported to the result pool (perCellFeatures) for VDJ Multiomic Integration. This
  // output resolves the workflow's collapsed perCellTable PFrame, undefined until the workflow emits it.
  //
  // Uses createPlDataTableV2 (columns passed directly via getPColumns), NOT V3. This frame is our OWN
  // self-contained, non-batch processColumn output, and createPlDataTableV3's discovery cannot render it:
  // the object (scoped-sources) form returns undefined whatever the anchor/maxHops config, and the
  ...
  // non-batch processColumn plus samples-and-data setup. retentive avoids blanking the grid on recompute.
  // withStatus feeds PlAgDataTableV2 the OutputWithStatus envelope it renders loading and error from.
  .output(
    "perCellTable",
    (ctx) => {
  ...
  // panel-assigned fraction. Uses createPlDataTableV2 like perCellTable, because it runs getAllLabelColumns
  // over the result pool and auto-joins the matching sampleId label. A V3 selector of
  // { mode: "enrichment", maxHops: 0 } never traverses to the upstream pl7.app/label column, and the
  // sampleId axis then renders the raw sample hash.
  .output(
    "qcSummaryTable",
    (ctx) => {
  ...
  // label the workflow put on its column. Two things on the card read it, and neither narrows anything: the
  // punch hover, because a reader hovering a dot far down a long grid cannot see the header row, and the
  // card's empty state, to tell "the pivot emitted no identity columns" apart from "this run has no rows".
  //
  ...
  // Reading the columns lists what the punchcard can actually draw.
  ...
  // filters in its filters panel, and this output second-guesses neither.
  //
  ...
  // discovers the label, the frame's only axis is consumed, and the engine is handed an empty key vector.
  // One axis is inherent to a punchcard, so there is nothing to tune. V3's `primaryColumns` form takes the
  // columns as given and runs NO data-column discovery, so it never walks the result pool and cannot hang
  // on the upstream Samples & Data FASTQ dataset -- the hazard the other tables here chose V2 to avoid, and
  // the reason this is not a blanket migration. V3 still resolves label columns for the axes it was handed,
  // which is wanted: a clonotype row reads better under its clonotype label than under a raw key.
  //
  // V2 is deprecated SDK-side in favour of this call, so the rest of this model's tables will follow. Each
  ...
  .output(
    ...
    (ctx) => {
      ...
      // grid. It is keyed on the same single axis as the punch columns, which is what makes adding it safe:
      // a column carrying an axis the others lack would widen the join.
      ...
      // Headers carry the identity's full name, never a truncation: which identity a column is, is the one
      // thing a reader needs from a header. Correct a too-long label where it is produced.
      //
      // Column ORDER comes from the `pl7.app/table/orderPriority` annotation on each spec, and from nothing
      // else. The cell count carries 96000, between the clonotype label's 100000 and the punches' 92000, and
      // lands at position 3: row number, clonotype, cell count, then the identities. A `displayOptions.ordering`
      // rule and this array's own order are BOTH inert here. If you are fixing a column that "renders last",
      // measure first, and measure with `aria-colindex`: `querySelectorAll('[role="columnheader"]')` returns
      // AG Grid's recycled header nodes in an order unrelated to column position.
  ...
  // The clonotype axis id, DERIVED from an emitted column and never written out by hand. The page hangs the
  // expansion's row button on this axis, and `showCellButtonForAxisId` is matched with `isJsonEqual` --
  // exact JSON equality, domain and all. A hand-written `{type, name}` misses the domain this axis carries,
  // matches nothing, and renders no button and no error. Deriving it from the same spec the filter reads
  // also makes the two provably agree.
  ...
  // The expansion: ONE clonotype's identities, read down. `the-explore-readout` puts this opposite the card
  // -- the grid is read across a row to see what a clone bound, and the expansion down to see what those
  // verdicts rest on, which is where a number belongs. A number in every position of the card would compete
  // with reading it across, so this is where cellsBound and the support counts surface.
  //
  // Reads `antigenVerdictsTable`: the LONG verdicts family at (set, identity) grain, held open by main.tpl
  // for exactly this, so this page costs no workflow change and no second import. Its rows are identities,
  // the shape the expansion wants and the shape a pivot cannot give it.
  //
  // NOT gated on the identity count that gates the card's pivots. That gate exists because a pivot costs a
  // COLUMN per identity. Here an identity costs a ROW, and only one clonotype's rows are ever fetched, so a
  // panel too wide for the card is precisely where this view still reads.
  //
  // The filter is pushed down, not applied after the fact: `createPlDataTableV3` puts it in the PTable def,
  // `createPTableDefV3` wraps the join in a `{type:"filter", predicate}` query node, and the engine lowers
  // that into the data query (pframes-rs `visit_filter`). One clonotype's rows are what crosses the
  // boundary, whatever the run's size.
  .output(
    "expansionTable",
    (ctx) => {
      // Undefined until a clonotype is chosen, and that is the point rather than a convenience: a table
      // built with no filter is EVERY clonotype's identities at once, which on a real run is the exact cost
      // this design exists to avoid. No selection, no table.
      ...
      // `antigenVerdictsTable` surfaces the entire export frame, whose families are keyed on five different
      // axes: tag, panel, sample, set, and (set, identity). Handing all of them to one table is a malformed
      // join, and the SDK answers `discoverColumns failed` out of `discoverLabelColumns`, which reads as an
      // SDK fault and is not one. Only the (set, identity) family belongs here.
      //
      // The identity's readable name comes FIRST, and it has to be named here rather than left to
      // `columns: null`. That option resolves label columns from the result pool, and this label lives in
      // `exportFb` -- a block's own exports are not in its own result pool. Without it every row prints the
      // same clonotype with nothing telling them apart.
      //
      // Could-answer is CONDITIONAL. Under one panel it is the clonotype's own cell count at every identity,
      // which the grid already carries beside its name, so a column of it repeats one number down the page.
      // The whole argument for carrying the number is that a verdict from three cells and one from forty
      // print the same word, and a number that never varies defeats that argument.
      //
      // Read from the run RECORD, not from current args: what panels a run carried is a fact about that run,
      // the same discipline the comparator uses (served, never requested).
      ...
      // column is `answered - bound` printed out, and a reader who wants it subtracts two numbers already on
      // the row. It stays in the EXPORT, which has its own readers. Only this panel drops it.
      const WANTED = [
        "pl7.app/label",
      ...
      // label fails to match -- an axis name drifting at export time is all it takes -- the verdict and bound
      // columns still match, the count is still non-zero, and the panel renders anonymous rows with nothing
      // to say it regressed. No panel is a visible failure. A nameless one reads as working.
      ...
      // `pCols[0].spec.axesSpec[0]` would hand the filter that axis and resolve nothing. An axis assembled
      // here would be a lookalike with a different identity and filter nothing.
      ...
      return createPlDataTableV3(ctx, {
        ...
        displayOptions: {
          visibility: [
            ...
            // Any other label column here labels the CLONOTYPE axis, and the panel is about one clonotype,
            // which the reader chose by clicking it. Printing its name down every row is repetition.
            // Optional rather than hidden, so the Columns picker can bring it back.
          ...
          // denominator reads before the number it divides. In this panel the bound count is what the reader
          // came for and the answered count is the context, so the two swap. Overridden here rather than in
          // the workflow spec because those columns are EXPORTS with downstream readers, and a priority is
          // global.
          //
          ...
          // expanded clonotype, so every clonotype opened once has its own frozen `columnOrder.orderedColIds`.
          // A stored order is an explicit list of column ids and it beats anything the model asks for. A
          // reorder that has to reach already-opened clonotypes therefore needs the cache invalidated, the
          // device the v3 -> v4 migration used. Not done here: the order is a preference, and resetting every
          // reader's saved columns and filters to move one column right is the more expensive mistake.
          ...
          filters: [
            {
              ...
              // unresolved-leaf path calls `console`, which does not exist in the model's QuickJS sandbox.
              // The symptom is then `ReferenceError: 'console' is not defined` from deep inside the SDK,
              // naming nothing about the filter. That error means an unresolvable filter column.
              column: {
                type: "axis",
                id:
            ...
            },
            {
              ...
              // A never-asked position is not a reading, and the panel keeps the numbers to the identities
              // the experiment actually put to these cells. Filtered by the verdict's own value rather than
  ...
  // verdict came out as it did: an `unreliable` on the card is cells disagreeing, and nothing but this shows
  // the disagreement.
  //
  ...
  //
  // Same push-down as the by-identity face, and it matters more here, where the frame's grain is every cell
  // of the run.
  .output(
    "cellExpansionTable",
    (ctx) => {
      ...
      // gated the pivot away, a legitimate state and not an error. So no table, and the page says why from
      // the run record.
      ...
      // No ordering rule, deliberately. The bound count sits immediately right of the axes because its own
      // annotation priority (95000) outranks every identity column (94000 and down), and that is where it
      // belongs: it is the one number that summarises the row, and a matrix a hundred columns wide puts its
      // far edge off screen. The by-identity face carries a rule only because there the annotation put
      // cells-answered in the wrong place.
      return createPlDataTableV3(ctx, {
  ...
  // The run's own quality report: every declared measurement with its status, the coverage triple behind it,
  // and -- where nothing computed it -- the reason it was deferred. This block is obliged to produce the
  ...
  // own result pool.
  //
  // `allowPermanentAbsence` for the same reason punchcardTable needs it: the whole verdict stage is gated on
  // a V(D)J dataset being picked, so on a run without one this field never appears, and a resolve that
  // treats a permanent absence as a pending one waits forever instead of returning undefined.
  //
  // A frame with no rows is deliberately NOT folded into undefined. Absent means the verdict stage did not
  // run. Empty means it ran and had nothing to report, which for the mismatch check is the good outcome.
  .output(
    "runQualityTable",
    (ctx) => {
      const pCols = ctx.outputs
  ...
  // report the user cannot see defeats its purpose, which is why the workflow emits it into `outputs` and
  // not only into the exports.
  .output(
  ...
  // What the reading was actually answered under. The page states the comparator that SERVED rather than the
  // one that was requested, because the software degrades a request it cannot honour and a reader meeting an
  // all-unreliable table otherwise has no way to learn that happened. Absent until a run with a V(D)J
  // dataset has produced it.
  ...
  // divergence between a shown rung and a sent one came from two copies of one rule. Nothing writes back, so
  // there is no hairpin.
  ...
  .retentiveOutput("referenceSources", (ctx): ReferenceSourceChoices => {
    ...
    // configuration `args()` refuses. The published view that a tag declared to be bound by nothing is not
    // truly negative is served by the two rungs that need no such tag, rather than by an option that
    // produces no answers.
  ...
  // outputs renders "Invalid subtitle" -- so the dynamic "<dataset> / <barcode> - <feature>" string is
  // derived in the `suggestedBlockLabel` OUTPUT, which HAS the pool, and copied into `defaultBlockLabel` by a
  // UI watchEffect. Guard `ctx.data`: it can be undefined before block storage is parsed.
  ...
  // results table -- appear only once the block has produced outputs, so an unrun block shows only Main.
